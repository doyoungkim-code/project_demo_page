...
프로젝트가 카드 갤러리로 펼쳐지며, 각 카드에는 실제 UI를 축소한 **움직이는 미니 프리뷰**(강의 자막 화면 / 폰 채팅)가 들어 있습니다.

- **검색 · 태그 필터** — 이름·소개·태그로 실시간 검색, 태그 칩으로 필터
- **라이트/다크 토글** — 선택을 `localStorage`에 저장 (첫 방문은 OS 설정을 따름)
- **스크롤 등장 애니메이션**, 모바일 1열 반응형, `prefers-reduced-motion` 대응
...
허브는 데이터로 카드를 생성합니다. [`index.html`](index.html)의 `PROJECTS` 배열에 객체 하나만 추가하면 카드가 생깁니다.
